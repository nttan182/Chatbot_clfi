...
class ActionXemDanhSachQuyDinh(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        try:
            ...
            if results:
                message = "Danh sách các quy định, quy chế và các văn bản phục vụ sinh viên hiện có: <br>"
        ...
        try:
            ...
            if result and result[0]:
                ...
                dispatcher.utter_message(
                    text=f"Hiện tại chưa có thông tin bạn đang hỏi")
                # return [SlotSet("danh_sach_quy_dinh", None)]
        except Exception as e:
...
class ActionTraCuuDieuKienBaoLuu(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[
        ...
        try:
            ...
            cursor.execute("""
                SELECT mo_ta 
                FROM dieu_kien_bao_luu 
                WHERE noi_dung ILIKE %s
            """, (f"%{khoa_hoc}%",))
